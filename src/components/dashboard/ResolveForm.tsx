...
export default function ResolveForm({
  ...
  return state === "done" ? (
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      ...
      <label className="mt-4 block text-xs font-medium text-slate-600">Problem</label>
      <textarea
        value={problem}
        onChange={(e) => setProblem(e.target.value)}
        rows={2}
        className="mt-1 w-full rounded-lg border border-slate-200 p-2.5 text-sm outline-none focus:border-indigo-400"
      />
      <label className="mt-3 block text-xs font-medium text-slate-600">Solution</label>
      <textarea
        value={solution}
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        {state === "error" ? (
          <p className="text-sm text-red-600">Failed - try again.</p>
        ) : null}
      </div>
    </div>
  );
